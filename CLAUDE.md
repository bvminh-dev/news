...
- OAuth token subscription Claude Code (`sk-ant-oat`) KHÔNG gọi được Messages API trực tiếp (SDK) — luôn `429 rate_limit_error`; muốn dùng token subscription phải **spawn `claude` CLI headless** (`claude -p`, ENV `CLAUDE_CODE_OAUTH_TOKEN`). API key `sk-ant-api` thì đi `ANTHROPIC_API_KEY`+`--bare` (i-20260707223448-ban-tin-hang-ngay).
- Adapter spawn CLI ⇒ KHÔNG chạy được trên serverless (Vercel): cần binary + spawn + FS tạm; nếu deploy serverless phải giữ đường SDK/API key hoặc chuyển host long-running — kiểm tra ràng buộc hosting trước khi chọn cơ chế gọi (i-20260707223448-ban-tin-hang-ngay).
- Khi spawn tiến trình con dùng credential/nội dung untrusted: bơm secret qua ENV (không qua arg), prompt qua stdin, `--permission-mode plan` (chỉ-đọc), `CLAUDE_CONFIG_DIR` tạm + xoá ENV auth kế thừa, dọn trong `finally` (i-20260707223448-ban-tin-hang-ngay).
- `claude` CLI ở chế độ `-p` ghi LỖI (auth/quota) ra **STDOUT** rồi exit≠0, không ghi stderr → phải **gộp stdout+stderr** khi phân loại lỗi kẻo bỏ sót nguyên nhân; đừng dựa mỗi stderr (i-20260707223448-ban-tin-hang-ngay).
- Parse JSON từ output LLM/CLI: dùng **quét ngoặc cân bằng** (thử nhiều vị trí `[`), KHÔNG regex tham lam `\[[\s\S]*\]` — model hay kèm prose/markdown fence làm `JSON.parse` hỏng (i-20260707223448-ban-tin-hang-ngay).
- Verify tính năng gọi dịch vụ ngoài phải chạy THẬT với credential thật (vd `npx tsx` gọi thẳng adapter), không chỉ dựa unit test mock — lỗi 429/auth/độ trễ chỉ lộ khi chạy live (i-20260707223448-ban-tin-hang-ngay).
<!-- tieu-nhi:end -->
